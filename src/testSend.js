...
async function runManualTest() {
    ...
    // 0. Firestore 초기화
    console.log('[0/3] Firestore 연결 중...');
    await initDatabase();

    // 1. 최신 기사 크롤링 (gap-bridging 포함)
    console.log('[1/3] 최신 기사 크롤링 중 (gap-bridging 적용)...');
    const articles = await getLatestArticles(10);

    console.log(`\n  총 ${articles.length}개 기사 수집 완료`);
    articles.forEach((a, i) => {
        console.log(`  ${i + 1}. [${a.category}] ${a.title.slice(0, 50)}... (${a.id})`);
    });
    console.log('');
